...
import com.ganjunhao.cache.AuthStateRedisCache;
...
public class RestAuthController {
    ...
    @ResponseBody
    public R refreshAuth(@PathVariable("source") String source, @PathVariable("uuid") String uuid) {
        AuthRequest authRequest = getAuthRequest(source.toLowerCase());
        ...
        try {
            ...
            if (response.ok()) {
                ...
                return R.ok("用户 [" + user.getUsername() + "] 的 access token 已刷新！新的 accessToken: " + response.getData().getAccessToken());
            }
            return R.fail("用户 [" + user.getUsername() + "] 的 access token 刷新失败！" + response.getMsg());
        } catch (AuthException e) {
            return R.fail(e.getErrorMsg());
        }
    }
        ...
        switch (source.toLowerCase()) {
            ...
            case "wechat_open":
                authRequest = new AuthWeChatOpenRequest(AuthConfig.builder()
                        .clientId("wxeca0dc53c1eaa5f1")
                        .clientSecret("a84e274e98ba6936d9370463196bd079")
                        .redirectUri("http://azik7s.natappfree.cc/oauth/callback/wechat_open")
                        .build());
                break;
            case "csdn":
            ...
            case "wechat_mp":
                authRequest = new AuthWeChatMpRequest(AuthConfig.builder()
                        .clientId("wxeca0dc53c1eaa5f1")
                        .clientSecret("a84e274e98ba6936d9370463196bd079")
                        .redirectUri("http://azik7s.natappfree.cc/oauth/callback/wechat_mp")
                        .build());
                break;
            case "aliyun":
